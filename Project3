...
from [master].[dbo].[sales_data_sample]
group by  DEALSIZE
order by 2 desc
...
----RFM: Rececy Frequency Monetary, an indexing techniques usepast purchase behaviour to segment customers 
----r: how long ago their last purchase, f: how often they purchase, m: how much they spent

DROP TABLE IF EXISTS #rfm --- window functions
;with rfm as 
(
	select 
		CUSTOMERNAME, 
